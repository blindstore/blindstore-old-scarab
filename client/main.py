...
class BlindstoreArray:
    ...
    def retrieve(self, index):
        ...
        """
        demo_log('Initiating request',
                 'Will ask for row number {}.'.format(index))
        ...
        demo_log('Index encrypted and ready to send',
        ...
        demo_log('Got response', str([str(x) for x in enc_data]))
        ret =  [secret_key.decrypt(bit) for bit in enc_data]
        demo_log('Response was decrypted :)', str(ret))
        return ret
    ...
    def set(self, index, data):
        ...
        """
        demo_log('Updating DB', 'Setting row number {} to {}.'
                 .format(index, data))
